/**
 * Call Stack Context Manager Plugin
 * @author: @irl_danb
 * @version: 1.0.0
 * @since: 2025-12-25
 */
...
interface FrameMetadata {
  ...
  // Planning and Invalidation fields
...
function getTokenBudget(): TokenBudget {
  ...
  return {
    ...
    ancestors: envAncestors
      ? parseInt(envAncestors, 10)
      : DEFAULT_TOKEN_BUDGET.ancestors,
    siblings: envSiblings
      ? parseInt(envSiblings, 10)
      : DEFAULT_TOKEN_BUDGET.siblings,
    current: envCurrent
      ? parseInt(envCurrent, 10)
      : DEFAULT_TOKEN_BUDGET.current,
...
function generateFrameCompactionPrompt(
  ...
  const completedSiblings = siblings.filter(
    (s) => s.status === "completed" && s.resultsCompacted
  );
...
function getCompactionType(sessionID: string): CompactionType {
  return (
    runtime.compactionTracking.compactionTypes.get(sessionID) || "overflow"
  );
}
...
// ============================================================================
// Subagent Integration Functions
// ============================================================================
...
function loadSubagentConfigFromEnv(): Partial<SubagentConfig> {
  ...
  if (process.env.STACK_SUBAGENT_MIN_MESSAGES) {
    config.minMessageCount = parseInt(
      process.env.STACK_SUBAGENT_MIN_MESSAGES,
      10
    );
  }
  if (process.env.STACK_SUBAGENT_AUTO_COMPLETE !== undefined) {
    config.autoCompleteOnIdle =
      process.env.STACK_SUBAGENT_AUTO_COMPLETE === "true";
  }
  if (process.env.STACK_SUBAGENT_IDLE_DELAY) {
    config.idleCompletionDelay = parseInt(
      process.env.STACK_SUBAGENT_IDLE_DELAY,
      10
    );
  }
  if (process.env.STACK_SUBAGENT_PATTERNS) {
    config.subagentPatterns = process.env.STACK_SUBAGENT_PATTERNS.split(
      ","
    ).map((s) => s.trim());
  }
...
function getSubagentStats(): SubagentStats & { activeSessions: number } {
  return {
    ...
    activeSessions: Array.from(
      runtime.subagentTracking.sessions.values()
...
function loadAutonomyConfigFromEnv(): Partial<AutonomyConfig> {
  ...
  if (process.env.STACK_PUSH_THRESHOLD) {
    config.pushThreshold = Math.min(
      100,
      Math.max(0, parseInt(process.env.STACK_PUSH_THRESHOLD, 10))
    );
  }
  if (process.env.STACK_POP_THRESHOLD) {
    config.popThreshold = Math.min(
      100,
      Math.max(0, parseInt(process.env.STACK_POP_THRESHOLD, 10))
    );
  }
  ...
  if (process.env.STACK_ENABLED_HEURISTICS) {
    config.enabledHeuristics = process.env.STACK_ENABLED_HEURISTICS.split(
      ","
    ).map((s) => s.trim());
  }
...
async function evaluatePushHeuristics(
  ...
  if (config.enabledHeuristics.includes("context_switch")) {
    ...
    if (potentialNewGoal && currentGoal) {
      ...
      const overlapRatio =
        currentKeywords.size > 0 ? overlap / currentKeywords.size : 0;
...
async function evaluatePopHeuristics(
  ...
  // Default context values
  const goalKeywords =
    context.goalKeywords ||
    (frame
      ? Array.from(extractKeywords(`${frame.title} ${frame.successCriteria}`))
      : []);
  ...
  if (config.enabledHeuristics.includes("goal_completion")) {
    ...
    const allText = [...recentArtifacts, ...successSignals]
      .join(" ")
      .toLowerCase();
    let keywordHits = 0;
  ...
  if (heuristicScores["stagnation"] > 60) {
    suggestedStatus =
      failureSignals.length > successSignals.length ? "failed" : "blocked";
 ...
 */
function scoreAncestor(
  ancestor: FrameMetadata,
  depth: number,
...
function scoreSibling(sibling: FrameMetadata, currentCriteria: string): number {
  ...
  const siblingWords = extractKeywords(
    `${sibling.title} ${sibling.successCriteriaCompacted}`
  );
  const resultsWords = sibling.resultsCompacted
    ? extractKeywords(sibling.resultsCompacted)
    : new Set<string>();
...
class FrameStateManager {
  ...
  /**
   * Load state (Alias for getAllFrames for UI consistency)
   */
  ...
  async ensureFrame(
    ...
    const defaultCriteria =
      "(Auto-created root frame - set criteria with a planning tool)";
  ...
  // ================================================================
  // Planning and Invalidation Methods
  // ================================================================

  /**
   * Create a planned frame (not yet started)
   * Planned frames exist in 'planned' status before activation
   */
   ...
   * Allows sketching out B→B1,B2,B3 before starting B
   */
   ...
   * Starts work on a previously planned frame
   */
  async activateFrame(sessionID: string): Promise<FrameMetadata | null> {
    ...
    if (frame.status !== "planned") {
      log("Frame is not in planned status", {
        sessionID,
        status: frame.status,
      });
      return null;
  ...
  async getAllSiblings(sessionID: string): Promise<FrameMetadata[]> {
    ...
    return Object.values(state.frames).filter(
      (f) =>
        f.parentSessionID === frame.parentSessionID && f.sessionID !== sessionID
    );
  }

  /**
   * Get all children of a frame (any status)
   * Enhanced to include all statuses for tree visualization
   */
   ...
   * Useful for finding all planned frames, etc.
   */
...
function calculateSiblingOrder(
  ...
  const position =
    allWithCurrent.findIndex((f) => f.sessionID === currentFrame.sessionID) + 1;
  ...
  for (const sibling of allWithCurrent) {
    ...
    if (sibling.status === "completed") {
      ...
      if (!nextPending) {
        const siblingPos =
          allWithCurrent.findIndex((f) => f.sessionID === sibling.sessionID) +
          1;
  ...
  if (!nextPending) {
    for (const sibling of allWithCurrent) {
      if (
        ...
        sibling.sessionID !== currentFrame.sessionID
      ) {
        nextPending = sibling;
        break;
      }
 ...
 * Includes token budget management, intelligent selection, and caching.
 */
async function generateFrameContext(
  manager: FrameStateManager,
  sessionID: string
): Promise<string> {
...
async function generateFrameContextWithMetadata(
  ...
  // Intelligent ancestor selection with budget
  const ancestorSelection = selectAncestors(
    allAncestors,
    budget.ancestors,
    frame
  );

  // Sibling relevance filtering with budget (use completed siblings for context)
  const siblingSelection = selectSiblings(
    completedSiblings,
    budget.siblings,
    `${frame.title} ${frame.successCriteria}`
  );
  ...
  const totalTokens =
    ancestorSelection.tokensUsed +
    siblingSelection.tokensUsed +
    currentTokens +
    budget.overhead;
...
function buildContextXml(
  ...
  if (ancestors.length > 0) {
    ...
    for (const ancestor of ancestors) {
      xml += formatFrameXml(
        ancestor,
        "    ",
        budget.ancestors / Math.max(ancestors.length, 1)
      );
    }
    xml += `  </ancestors>\n`;
  }
  ...
  if (siblings.length > 0) {
    ...
    for (const sibling of siblings) {
      xml += formatFrameXml(
        sibling,
        "    ",
        budget.siblings / Math.max(siblings.length, 1)
      );
    }
  ...
  const currentFrameXml = formatCurrentFrameXml(
    frame,
    "  ",
    budget.current,
    plannedChildren
 ...
 */
function formatFrameXml(
  frame: FrameMetadata,
  indent: string,
  tokenBudgetPerFrame: number
): string {
 ...
 * Includes planned children to show what work is planned next.
 */
function formatCurrentFrameXml(
  ...
  if (frame.decisions.length > 0) {
    ...
    const { text: truncatedDecisions, wasTruncated: decisionsTruncated } =
...
export const StackPlugin: Plugin = async (ctx) => {
  ...
  // Initialize token budget from environment
  runtime.tokenBudget = getTokenBudget();
  // Initialize subagent config from environment
  const envSubagentConfig = loadSubagentConfigFromEnv();
  runtime.subagentTracking.config = {
    ...DEFAULT_SUBAGENT_CONFIG,
    ...envSubagentConfig,
  };
  // Initialize autonomy config from environment
  const envAutonomyConfig = loadAutonomyConfigFromEnv();
  runtime.autonomyTracking.config = {
    ...DEFAULT_AUTONOMY_CONFIG,
    ...envAutonomyConfig,
  };

  log("=== STACK PLUGIN INITIALIZED ===");
  log("Plugin context", {
  ...
  return {
    ...
    event: async ({ event }) => {
      ...
      if (event.type === "session.created" && "info" in event.properties) {
        ...
        // Enhanced subagent detection and tracking
        if (info.parentID) {
      ...
      if (event.type === "session.idle" && "sessionID" in event.properties) {
        ...
        // Handle subagent session idle
        if (runtime.subagentTracking.config.enabled) {
          const subagentSession =
            runtime.subagentTracking.sessions.get(sessionID);
      ...
      if (
        ...
        const pendingCompletion =
          runtime.compactionTracking.pendingCompletions.get(sessionID);

        log("SESSION COMPACTED", {
          sessionID,
          compactionType,
        ...
        try {
          ...
          const summaryMessage = messages.data
            ?.filter(
              (m: { info: { summary?: boolean } }) => m.info.summary === true
            )
            .pop();
          ...
          if (pendingCompletion) {
            try {
              ...
              clearCompactionTracking(sessionID);
              log(
                "Frame completion finalized despite summary extraction failure",
                {
                  sessionID,
                }
              );
    ...
    "experimental.chat.system.transform": async (input, output) => {
      ...
      // Append autonomy suggestions to context if enabled
    ...
    /**
     * experimental.session.compacting - customize compaction prompt for frames
     *
    ...
    "experimental.session.compacting": async (input, output) => {
      ...
      if (frame) {
        ...
        if (
          compactionType === "frame_completion" ||
          compactionType === "manual_summary"
        ) {
        ...
        log("Compaction context added for frame", {
          sessionID: input.sessionID,
          title: frame.title,
          compactionType,
     ...
     * Automatically adds modified files to the current frame's artifacts
     */
    ...
    tool: {
      ...
      stack_frame_push: tool({
        description: `Create a new child frame for a subtask. Use when starting a distinct unit of work that could be retried or rolled back independently.
...
- Can be a few sentences, but should maximize information density`,
        args: {
          title: tool.schema
            .string()
            .describe(
              "Short name for the frame (e.g., 'User Authentication', 'API Endpoints')"
            ),
          successCriteria: tool.schema
            .string()
            .describe(
              "Full success criteria - what defines 'done' for this frame. Be specific about deliverables."
            ),
          successCriteriaCompacted: tool.schema
            .string()
            .describe(
              "Dense compacted version for tree display. Preserve specifics, don't generalize."
            ),
        },
...
Work on this subtask, then use stack_frame_pop to complete it and return to the parent frame.`;
      ...
      stack_frame_pop: tool({
        description: `Complete a frame and return to its parent. Requires both full results and a compacted version.
...
GOOD EXAMPLE (compacted): "Auth endpoints complete: POST /register, /login, /logout in src/auth/. JWT w/ 24h expiry, bcrypt passwords. Tests passing. Blocked: SMTP config needed for password reset."`,
        args: {
          frameID: tool.schema
            ...
            .optional()
            .describe(
              "Specific frame ID to complete. If not provided, uses current session."
            ),
          status: tool.schema
          ...
          results: tool.schema
            .string()
            .describe(
              "Full detailed results: what was accomplished, decisions made, current state, blockers."
            ),
          resultsCompacted: tool.schema
            .string()
            .describe(
              "Dense compacted version for tree display. Preserve specifics, don't generalize. This appears in sibling/parent context."
            ),
        },
        async execute(args, toolCtx) {
          ...
          if (
            ...
            args.resultsCompacted.trim().length === 0
          ) {
...
This is now available as context for sibling frames and the parent.`;
      ...
      stack_status: tool({
        ...
        async execute(args, toolCtx) {
          ...
          const orphans = Object.values(state.frames).filter(
            (f) =>
              !f.parentSessionID && !state.rootFrameIDs.includes(f.sessionID)
          );
          if (orphans.length > 0) {
            output += "\n(Orphaned frames):\n";
      ...
      stack_frame_details: tool({
        ...
        args: {
          frameID: tool.schema
            .string()
            .describe("The frame ID to show details for"),
        },
        async execute(args, toolCtx) {
          if (!args.frameID) {
      ...
      stack_add_artifact: tool({
        description:
          "Record an artifact (file, resource, etc) produced by this frame",
        args: {
          artifact: tool.schema
            .string()
            .describe("The artifact path or description"),
        },
        async execute(args, toolCtx) {
          const sessionID = runtime.currentSessionID;
          if (!sessionID) {
          ...
          // Invalidate cache (artifacts affect sibling relevance scoring)
          invalidateCache(sessionID);
      ...
      stack_add_decision: tool({
        ...
        async execute(args, toolCtx) {
          ...
          // Invalidate cache (decisions affect context content)
          invalidateCache(sessionID);
      ...
      /**
       * /stack-context-info - Show context generation metadata
       */
      stack_context_info: tool({
        description:
          "Show context generation metadata including token usage, caching info, and what was included/filtered",
        args: {},
        async execute(args, toolCtx) {
          const sessionID = runtime.currentSessionID;
          if (!sessionID) {
          ...
          const result = await generateFrameContextWithMetadata(
            manager,
            sessionID
          );

          let output = "# Stack Context Assembly Info\n\n";
      ...
      /**
       * /stack-context-preview - Preview the actual XML context that would be injected
       */
      stack_context_preview: tool({
        description:
          "Preview the actual XML context that would be injected into LLM calls",
        args: {
      ...
      /**
       * /stack-cache-clear - Clear the context cache
       */
      stack_cache_clear: tool({
        description:
          "Clear the context cache for all sessions or a specific session",
        args: {
          sessionID: tool.schema
            .string()
            .optional()
            .describe(
              "Optional session ID to clear cache for (clears all if not provided)"
            ),
        },
        async execute(args, toolCtx) {
          if (args.sessionID) {
      ...
      stack_frame_summarize: tool({
        ...
        args: {
          note: tool.schema
            ...
            .optional()
            .describe(
              "Optional note to include in the summary prompt (e.g., 'checkpoint before refactoring')"
            ),
        },
        async execute(args, toolCtx) {
          const sessionID = runtime.currentSessionID;
      ...
      /**
       * /stack-compaction-info - Show compaction tracking state
       */
      stack_compaction_info: tool({
        description:
          "Show current compaction tracking state including pending completions and compaction types",
        args: {},
        async execute(args, toolCtx) {
          const sessionID = runtime.currentSessionID;

          let output = "# Stack Compaction Tracking Info\n\n";
          ...
          if (sessionID) {
            ...
            const pendingCompletion =
              runtime.compactionTracking.pendingCompletions.get(sessionID);
            const isPending =
              runtime.compactionTracking.pendingCompactions.has(sessionID);
          ...
          if (runtime.compactionTracking.pendingCompactions.size > 0) {
            ...
            runtime.compactionTracking.pendingCompactions.forEach((sid) => {
              const type =
                runtime.compactionTracking.compactionTypes.get(sid) ||
                "unknown";
      ...
      stack_get_summary: tool({
        description:
          "Get the compaction summary for the current frame or a specific frame",
        args: {
          sessionID: tool.schema
            .string()
            .optional()
            .describe(
              "Optional session ID (uses current frame if not provided)"
            ),
        },
        async execute(args, toolCtx) {
      ...
      stack_config: tool({
        description:
          "View or modify subagent integration settings. Shows current configuration and allows changing settings.",
        args: {
          enabled: tool.schema
            .boolean()
            .optional()
        ...
        async execute(args, toolCtx) {
          ...
          // Build output
          let output = `# Subagent Integration Configuration\n\n`;
      ...
      stack_stats: tool({
        description:
          "Show statistics about detected subagent sessions, including counts, completion rates, and active sessions.",
        args: {
          reset: tool.schema
            .boolean()
            .optional()
          ...
          showActive: tool.schema
            ...
            .optional()
            .describe(
              "Show details of all active (non-completed) sessions (default: true)"
            ),
        },
        async execute(args, toolCtx) {
          if (args.reset) {
          ...
          let output = `# Subagent Integration Statistics\n\n`;
          ...
          if (stats.totalDetected > 0) {
            const frameRate = (
              (stats.framesCreated / stats.totalDetected) *
              100
            ).toFixed(1);
            const skipRate = (
              (stats.skippedByHeuristics / stats.totalDetected) *
              100
            ).toFixed(1);
            const autoCompleteRate =
              stats.framesCreated > 0
      ...
      stack_subagent_complete: tool({
        description:
          "Manually complete a subagent session. Can be used to complete sessions that haven't auto-completed.",
        args: {
          sessionID: tool.schema
            .string()
            .optional()
            .describe(
              "Session ID to complete (uses current session if not provided)"
            ),
          status: tool.schema
            .enum(["completed", "failed", "blocked"])
            .optional()
        ...
        async execute(args, toolCtx) {
          ...
          const success = await completeSubagentSession(
            sessionID,
            status,
            args.summary,
            manager
...
The parent frame (${subagentSession.parentSessionID.substring(0, 8)}) will now include this session's context in its sibling summaries.`;
      ...
      stack_subagent_list: tool({
        description:
          "List all tracked subagent sessions with their current status and metadata.",
        args: {
          filter: tool.schema
        ...
        async execute(args, toolCtx) {
          ...
          let output = `# Stack Subagent Sessions\n\n`;
      ...
      // ================================================================
      // Planning and Invalidation Tools
      // ================================================================

      /**
       * stack_frame_plan - Create a planned frame (not started yet)
       * Frames can exist in 'planned' state before execution
       */
...
- Think "compression" not "summarization"`,
        args: {
          title: tool.schema
            .string()
            .describe("Short name for the frame (2-5 words)"),
          successCriteria: tool.schema
            .string()
            .describe(
              "Full success criteria - what defines 'done' for this frame"
            ),
          successCriteriaCompacted: tool.schema
            .string()
            .describe(
              "Dense compacted version of success criteria for tree display"
            ),
          parentSessionID: tool.schema
            .string()
            .optional()
            .describe(
              "Parent frame session ID (uses current frame if not provided)"
            ),
        },
        async execute(args, toolCtx) {
...
- Think "compression" not "summarization"`,
          ...
          children: tool.schema
            .array(
              tool.schema.object({
                ...
                successCriteria: tool.schema
                  .string()
                  .describe("Full success criteria"),
                successCriteriaCompacted: tool.schema
                  .string()
        ...
        async execute(args, toolCtx) {
          ...
          try {
            ...
            const frames = await manager.createPlannedChildren(
              parentID,
              childrenWithIDs
            );
       ...
       * Creates a real OpenCode session and replaces the plan-* ID
       */
      stack_frame_activate: tool({
        ...
        async execute(args, toolCtx) {
          ...
          try {
            ...
            const callStackContext = await generateFrameContext(
              manager,
              newSessionID
            );
...
Begin working on your task now.`;
            ...
            client.session
              ...
              .then(() => {
                log(
                  "ACTIVATE: Initial prompt sent to session with call-stack context",
                  { newSessionID }
                );
              })
              .catch((promptError) => {
...
A new OpenCode session has been created. Workflow guidance is provided in the system prompt.`;
      ...
      stack_frame_invalidate: tool({
        ...
        args: {
          sessionID: tool.schema
            ...
            .optional()
            .describe(
              "The session ID of the frame to invalidate (uses current frame if not provided)"
            ),
          reason: tool.schema
            .string()
            .describe(
              "The reason for invalidation (will be stored in frame metadata)"
            ),
        },
        async execute(args, toolCtx) {
          const sessionID = args.sessionID || runtime.currentSessionID;
      ...
      stack_tree: tool({
        ...
        args: {
          ...
          rootID: tool.schema
            ...
            .optional()
            .describe(
              "Optional root frame ID to start from (shows subtree only)"
            ),
          showDetails: tool.schema
            .boolean()
            .optional()
            .describe(
              "Show additional details like timestamps and summaries (default: false)"
            ),
        },
        async execute(args, toolCtx) {
          const state = await manager.getAllFrames();
          ...
          function buildTree(
            ...
            if (showDetails) {
              ...
              if (frame.resultsCompacted) {
                const resultsPreview = frame.resultsCompacted
                  .substring(0, 80)
                  .replace(/\n/g, " ");
          ...
          const stats = {
            ...
            completed: Object.values(state.frames).filter(
              (f) => f.status === "completed"
            ).length,
            in_progress: Object.values(state.frames).filter(
              (f) => f.status === "in_progress"
            ).length,
            planned: Object.values(state.frames).filter(
              (f) => f.status === "planned"
            ).length,
            invalidated: Object.values(state.frames).filter(
              (f) => f.status === "invalidated"
            ).length,
            blocked: Object.values(state.frames).filter(
              (f) => f.status === "blocked"
            ).length,
            failed: Object.values(state.frames).filter(
              (f) => f.status === "failed"
            ).length,
          };
       ...
       * Controls how autonomous the agent is in managing frames
       */
      stack_autonomy: tool({
        description:
        ...
        args: {
          level: tool.schema
            ...
            .optional()
            .describe(
              "Autonomy level: manual (never auto), suggest (suggest but wait), auto (act automatically)"
            ),
          pushThreshold: tool.schema
            .number()
            .optional()
            .describe(
              "Confidence threshold (0-100) for auto-push recommendations"
            ),
          popThreshold: tool.schema
            .number()
            .optional()
            .describe(
              "Confidence threshold (0-100) for auto-pop recommendations"
            ),
          ...
          enableHeuristic: tool.schema
            ...
            .optional()
            .describe(
              "Enable a specific heuristic (e.g., 'failure_boundary', 'context_switch')"
            ),
...
**Enabled Heuristics:** ${DEFAULT_AUTONOMY_CONFIG.enabledHeuristics.join(", ")}`;
          ...
          if (args.disableHeuristic) {
            const index = config.enabledHeuristics.indexOf(
              args.disableHeuristic
            );
            if (index > -1) {
          ...
          // Build output
          let output = `# Agent Autonomy Configuration\n\n`;
       ...
       * Determines if current context warrants creating a new frame
       */
      stack_should_push: tool({
        ...
        args: {
          potentialGoal: tool.schema
            ...
            .optional()
            .describe(
              "Potential goal for the new frame (used for context switch detection)"
            ),
        ...
        async execute(args, toolCtx) {
          ...
          for (const [heuristic, score] of Object.entries(
          ...
          if (config.level !== "manual" && result.shouldPush) {
            ...
            if (
              config.level === "auto" &&
              result.confidence >= config.pushThreshold
            ) {
      ...
      stack_should_pop: tool({
        ...
        args: {
          successSignals: tool.schema
            ...
            .optional()
            .describe(
              "List of success indicators (e.g., 'tests passing', 'build succeeded')"
            ),
          failureSignals: tool.schema
            .array(tool.schema.string())
            .optional()
            .describe(
              "List of failure indicators (e.g., 'tests failing', 'error occurred')"
            ),
          noProgressTurns: tool.schema
            .number()
            .optional()
            .describe(
              "Number of conversation turns with no meaningful progress"
            ),
          tokenCount: tool.schema
            .number()
            .optional()
        ...
        async execute(args, toolCtx) {
          ...
          if (
            ...
            if (
              config.level === "auto" &&
              result.confidence >= config.popThreshold
            ) {
       ...
       * Controls whether suggestions are injected into context
       */
      stack_auto_suggest: tool({
        ...
        async execute(args, toolCtx) {
          ...
          let output = `# Auto-Suggestion System\n\n`;
          ...
          if (args.showHistory) {
            ...
            const history =
              runtime.autonomyTracking.suggestionHistory.slice(-20);
            if (history.length === 0) {
              ...
              for (const s of history.reverse()) {
                const truncSuggestion =
                  s.suggestion.length > 30
       ...
       * Used by the Stack UI to fetch the complete state tree
       */
       ...
       * Shows statistics about autonomy system usage
       */
      stack_autonomy_stats: tool({
        ...
        async execute(args, toolCtx) {
          ...
          let output = `# Autonomy Statistics\n\n`;
          ...
          if (stats.totalSuggestions > 0) {
            const actionRate = (
              (stats.actedUpon / stats.totalSuggestions) *
              100
            ).toFixed(1);
