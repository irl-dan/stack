/**
 * Flame Graph Context Management Plugin - Phase 1.7
 *
 ...
 * - flame_subagent_config, flame_subagent_stats tools
 ...
 * - flame_autonomy_config, flame_should_push, flame_should_pop, flame_auto_suggest tools
 */
...
function getDefaultState(): FlameState {
  return {
    version: 1,
    frames: {},
...
function log(message: string, data?: unknown): void {
  ...
  const entry = `[${timestamp}] [flame] ${message}`
...
function getFramesDir(projectDir: string): string {
  return path.join(getFlameDir(projectDir), "frames")
}

function getStateFilePath(projectDir: string): string {
  return path.join(getFlameDir(projectDir), "state.json")
}
...
async function loadState(projectDir: string): Promise<FlameState> {
  const statePath = getStateFilePath(projectDir)
  try {
    const content = await fs.promises.readFile(statePath, "utf-8")
    const state = JSON.parse(content) as FlameState
    return state
  } catch (error) {
...
async function saveState(projectDir: string, state: FlameState): Promise<void> {
...
function getTokenBudget(): TokenBudget {
  ...
  const envTotal = process.env.FLAME_TOKEN_BUDGET_TOTAL
  const envAncestors = process.env.FLAME_TOKEN_BUDGET_ANCESTORS
  const envSiblings = process.env.FLAME_TOKEN_BUDGET_SIBLINGS
  const envCurrent = process.env.FLAME_TOKEN_BUDGET_CURRENT
...
function generateFrameCompactionPrompt(
  ...
  let prompt = `## Flame Frame Compaction\n\n`
...
function loadSubagentConfigFromEnv(): Partial<SubagentConfig> {
  ...
  if (process.env.FLAME_SUBAGENT_ENABLED !== undefined) {
    config.enabled = process.env.FLAME_SUBAGENT_ENABLED === 'true'
  }
  if (process.env.FLAME_SUBAGENT_MIN_DURATION) {
    config.minDuration = parseInt(process.env.FLAME_SUBAGENT_MIN_DURATION, 10)
  }
  if (process.env.FLAME_SUBAGENT_MIN_MESSAGES) {
    config.minMessageCount = parseInt(process.env.FLAME_SUBAGENT_MIN_MESSAGES, 10)
  }
  if (process.env.FLAME_SUBAGENT_AUTO_COMPLETE !== undefined) {
    config.autoCompleteOnIdle = process.env.FLAME_SUBAGENT_AUTO_COMPLETE === 'true'
  }
  if (process.env.FLAME_SUBAGENT_IDLE_DELAY) {
    config.idleCompletionDelay = parseInt(process.env.FLAME_SUBAGENT_IDLE_DELAY, 10)
  }
  if (process.env.FLAME_SUBAGENT_PATTERNS) {
    config.subagentPatterns = process.env.FLAME_SUBAGENT_PATTERNS.split(',').map(s => s.trim())
  }
...
function loadAutonomyConfigFromEnv(): Partial<AutonomyConfig> {
  ...
  if (process.env.FLAME_AUTONOMY_LEVEL) {
    const level = process.env.FLAME_AUTONOMY_LEVEL.toLowerCase()
    if (level === 'manual' || level === 'suggest' || level === 'auto') {
      config.level = level
    }
  }
  if (process.env.FLAME_PUSH_THRESHOLD) {
    config.pushThreshold = Math.min(100, Math.max(0, parseInt(process.env.FLAME_PUSH_THRESHOLD, 10)))
  }
  if (process.env.FLAME_POP_THRESHOLD) {
    config.popThreshold = Math.min(100, Math.max(0, parseInt(process.env.FLAME_POP_THRESHOLD, 10)))
  }
  if (process.env.FLAME_SUGGEST_IN_CONTEXT !== undefined) {
    config.suggestInContext = process.env.FLAME_SUGGEST_IN_CONTEXT === 'true'
  }
  if (process.env.FLAME_ENABLED_HEURISTICS) {
    config.enabledHeuristics = process.env.FLAME_ENABLED_HEURISTICS.split(',').map(s => s.trim())
  }
...
function formatSuggestionsForContext(): string {
  ...
  for (const suggestion of suggestions) {
    if (suggestion.type === 'push') {
      output += `[FLAME SUGGESTION: Consider pushing a new frame for "${suggestion.suggestion}" - Reason: ${suggestion.reason} (${suggestion.confidence}% confidence)]\n`
    } else {
      output += `[FLAME SUGGESTION: Consider popping current frame with status "${suggestion.suggestion}" - Reason: ${suggestion.reason} (${suggestion.confidence}% confidence)]\n`
    }
  }
...
class FrameStateManager {
   ...
   * Get all frames
   */
  async getAllFrames(): Promise<FlameState> {
   ...
   */
  async loadState(): Promise<FlameState> {
    return await loadState(this.projectDir)
  }
...
export const FlamePlugin: Plugin = async (ctx) => {
  const { project, client, directory } = ctx

  runtime.flameDir = getFlameDir(directory)
  ...
  log("=== FLAME PLUGIN INITIALIZED (Phase 1.7) ===")
  log("Plugin context", {
    projectId: project.id,
    directory,
    flameDir: runtime.flameDir,
  ...
  return {
     ...
     * - Frame completion (via flame_pop)
     * - Manual summary (via flame_summarize)
...
This is now available as context for sibling frames and the parent.`
      ...
      flame_status: tool({
        ...
        async execute(args, toolCtx) {
          ...
          if (Object.keys(state.frames).length === 0) {
            return "No frames exist yet. Use flame_push to create the first frame."
          }

          let output = "# Flame Frame Tree\n\n"
      ...
      /**
       * /flame-add-artifact - Record an artifact produced by this frame
       */
      flame_add_artifact: tool({
      ...
      /**
       * /flame-add-decision - Record a key decision made in this frame
       */
      flame_add_decision: tool({
      ...
      /**
       * /flame-context-info - Show context generation metadata (Phase 1.2)
       */
      flame_context_info: tool({
        ...
        async execute(args, toolCtx) {
          ...
          let output = "# Flame Context Assembly Info (Phase 1.2)\n\n"
          ...
          output += `- FLAME_TOKEN_BUDGET_TOTAL: ${process.env.FLAME_TOKEN_BUDGET_TOTAL || "(not set)"}\n`
          output += `- FLAME_TOKEN_BUDGET_ANCESTORS: ${process.env.FLAME_TOKEN_BUDGET_ANCESTORS || "(not set)"}\n`
          output += `- FLAME_TOKEN_BUDGET_SIBLINGS: ${process.env.FLAME_TOKEN_BUDGET_SIBLINGS || "(not set)"}\n`
          output += `- FLAME_TOKEN_BUDGET_CURRENT: ${process.env.FLAME_TOKEN_BUDGET_CURRENT || "(not set)"}\n`
      ...
      /**
       * /flame-context-preview - Preview the actual XML context that would be injected (Phase 1.2)
       */
      flame_context_preview: tool({
        ...
        async execute(args, toolCtx) {
          ...
          let output = "# Flame Context Preview\n\n"
      ...
      /**
       * /flame-cache-clear - Clear the context cache (Phase 1.2)
       */
      flame_cache_clear: tool({
      ...
      /**
       * /flame-summarize - Manually trigger summary generation for current frame
       *
      ...
      /**
       * /flame-compaction-info - Show compaction tracking state (Phase 1.3)
       */
      flame_compaction_info: tool({
        description: "Show current compaction tracking state including pending completions and compaction types",
        args: {},
        async execute(args, toolCtx) {
          const sessionID = runtime.currentSessionID

          let output = "# Flame Compaction Tracking Info (Phase 1.3)\n\n"
      ...
      /**
       * /flame-get-summary - Retrieve the current compaction summary for a frame
       */
      flame_get_summary: tool({
      ...
      /**
       * /flame-subagent-config - View and modify subagent integration settings
       */
      flame_subagent_config: tool({
        ...
        async execute(args, toolCtx) {
          ...
          output += `- \`FLAME_SUBAGENT_ENABLED\`: ${process.env.FLAME_SUBAGENT_ENABLED || '(not set)'}\n`
          output += `- \`FLAME_SUBAGENT_MIN_DURATION\`: ${process.env.FLAME_SUBAGENT_MIN_DURATION || '(not set)'}\n`
          output += `- \`FLAME_SUBAGENT_MIN_MESSAGES\`: ${process.env.FLAME_SUBAGENT_MIN_MESSAGES || '(not set)'}\n`
          output += `- \`FLAME_SUBAGENT_AUTO_COMPLETE\`: ${process.env.FLAME_SUBAGENT_AUTO_COMPLETE || '(not set)'}\n`
          output += `- \`FLAME_SUBAGENT_IDLE_DELAY\`: ${process.env.FLAME_SUBAGENT_IDLE_DELAY || '(not set)'}\n`
          output += `- \`FLAME_SUBAGENT_PATTERNS\`: ${process.env.FLAME_SUBAGENT_PATTERNS || '(not set)'}\n`
      ...
      /**
       * /flame-subagent-stats - Show statistics about detected subagent sessions
       */
      flame_subagent_stats: tool({
      ...
      /**
       * /flame-subagent-complete - Manually complete a subagent session
       */
      flame_subagent_complete: tool({
...
The parent frame (${subagentSession.parentSessionID.substring(0, 8)}) will now include this session's context in its sibling summaries.`
      ...
      /**
       * /flame-subagent-list - List all tracked subagent sessions
       */
      flame_subagent_list: tool({
        ...
        async execute(args, toolCtx) {
          ...
          let output = `# Subagent Sessions (Phase 1.5)\n\n`
      ...
      /**
       * flame_plan - Create a planned frame (not started yet)
...
Planned frames will be automatically invalidated if their parent is invalidated.`
      ...
      /**
       * flame_plan_children - Create multiple planned children at once
       * Phase 1.6: Allows sketching out B→B1,B2,B3 before starting B
       */
...
A new OpenCode session has been created. Workflow guidance is provided in the system prompt.`
      ...
      /**
       * flame_invalidate - Invalidate a frame with cascade to planned children
       * Phase 1.6: When a frame is invalidated:
       *   - All 'planned' children are auto-invalidated
       *   - 'in_progress' children are warned but not auto-invalidated
       *   - 'completed' children remain completed
       */
      flame_invalidate: tool({
        description:
        ...
        async execute(args, toolCtx) {
          ...
          output += `Use \`flame_tree\` to see the updated frame structure.`
      ...
      flame_tree: tool({
        ...
        async execute(args, toolCtx) {
          ...
          if (Object.keys(state.frames).length === 0) {
            return "No frames exist yet. Use flame_push or flame_plan to create frames."
          }
          ...
          // Build tree output
          let output = `# Flame Frame Tree\n\n`
          output += `**Legend:** ✓ completed, → in_progress, ○ planned, ✗ invalidated, ! blocked, ⚠ failed\n`
          output += `**Active Frame:** ${state.activeFrameID?.substring(0, 8) || "none"}\n\n`
          output += "```\n"
      ...
      /**
       * flame_autonomy_config - View and modify autonomy settings
       * Phase 1.7: Controls how autonomous the agent is in managing frames
       */
      flame_autonomy_config: tool({
        description:
...
**Enabled Heuristics:** ${DEFAULT_AUTONOMY_CONFIG.enabledHeuristics.join(', ')}`
          ...
          output += `\n## Environment Variables\n`
          output += `- \`FLAME_AUTONOMY_LEVEL\`: ${process.env.FLAME_AUTONOMY_LEVEL || '(not set)'}\n`
          output += `- \`FLAME_PUSH_THRESHOLD\`: ${process.env.FLAME_PUSH_THRESHOLD || '(not set)'}\n`
          output += `- \`FLAME_POP_THRESHOLD\`: ${process.env.FLAME_POP_THRESHOLD || '(not set)'}\n`
          output += `- \`FLAME_SUGGEST_IN_CONTEXT\`: ${process.env.FLAME_SUGGEST_IN_CONTEXT || '(not set)'}\n`
          output += `- \`FLAME_ENABLED_HEURISTICS\`: ${process.env.FLAME_ENABLED_HEURISTICS || '(not set)'}\n`
        ...
        async execute(args, toolCtx) {
          ...
          if (config.level !== 'manual' && result.shouldPush) {
            ...
            if (config.level === 'auto' && result.confidence >= config.pushThreshold) {
              output += `\n\n**Auto Mode:** Consider using flame_push with goal "${result.suggestedGoal || 'New subtask'}"`
        ...
        async execute(args, toolCtx) {
          ...
          if (config.level !== 'manual' && result.shouldPop && frame.parentSessionID) {
            ...
            if (config.level === 'auto' && result.confidence >= config.popThreshold) {
              output += `\n\n**Auto Mode:** Consider using flame_pop with status "${result.suggestedStatus}"`
      ...
      /**
       * flame_auto_suggest - Toggle and manage auto-suggestions
       * Phase 1.7: Controls whether suggestions are injected into context
       */
      flame_auto_suggest: tool({
        description:
      ...
      /**
       * flame_get_state - Get complete flame state for UI rendering
       * Phase 2.1: Used by the Flame Graph UI to fetch the complete state tree
       */
      ...
      /**
       * flame_autonomy_stats - View detailed autonomy statistics
       * Phase 1.7: Shows statistics about autonomy system usage
       */
      flame_autonomy_stats: tool({
        description:
